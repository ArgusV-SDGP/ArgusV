...
down_revision: Union[str, Sequence[str], None] = 'e99301c897a2'
...
def upgrade() -> None:
    ...
    Uses conditional checks so the migration is idempotent and works both
    for fresh Alembic-only installs and for databases where SQLAlchemy's
    create_all() already created the base tables.
    """
    bind = op.get_bind()
    ...
    if 'zones' not in existing:
        ...
        if 'updated_at' not in zone_cols:
    ...
    if 'detections' not in existing:
        ...
        sa.Column('vlm_embedding', Vector(384), nullable=True),
        sa.ForeignKeyConstraint(['camera_id'], ['cameras.camera_id'], ),
        ...
        if 'vlm_embedding' not in det_cols:
...
def downgrade() -> None:
    ...
    if 'detections' in existing:
        det_cols = {c['name'] for c in inspector.get_columns('detections')}
        if 'vlm_embedding' in det_cols:
    ...
    if 'zones' in existing:
        zone_cols = {c['name'] for c in inspector.get_columns('zones')}
        if 'updated_at' in zone_cols:
            op.drop_column('zones', 'updated_at')
        op.drop_table('zones')
    if 'rag_configs' in existing:
