...
class ZoneMatcher:
    ...
    def _load_from_db(self):
        try:
            ...
            with engine.connect() as conn:
                rows = conn.execute(
                    text("SELECT zone_id, name, polygon_coords, allowed_classes, active FROM zones")
                ).fetchall()
    ...
    def match(self, cx_norm: float, cy_norm: float, object_class: str) -> Optional[dict]:
        """
        ...
        with self._lock:
            ...
            if not self._zones:
                return {"id": "default", "name": "Full Frame",
                        "polygon_coords": [], "allowed_classes": None}
        return None
...
class DetectLoop:
    ...
    def _process_results(self, frame: np.ndarray, timestamp: float, result):
        ...
        for box in result.boxes:
            ...
            zone = self._zones.match(cx_norm, cy_norm, object_class)
            if zone is None:
                continue
